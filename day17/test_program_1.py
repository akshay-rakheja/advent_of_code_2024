...
def parse_input(filename: str) -> tuple[State, list[int]]:
...
def find_valid_a(program: list[int], acc: int = 0, position: int = 0) -> Iterator[int]:
    # Add debug printing
    print(f"Trying position {position} with acc {acc}")

    if position == len(program):
        print(f"Found solution: {acc}")
        yield acc
        return
...
def main():
    initial_state, program = parse_input("day17/input_test.txt")
